...
extension HubApi {
        ...
        async throws -> URL
    {
        ...
        // LocalAssistant-modification: offline mode is not negotiable. A caller passing
        // `useOfflineMode: false` can no longer re-enable downloading.
        if NetworkState.shouldUseOfflineMode() {
...
// LocalAssistant-modification: replaced the NWPathMonitor-based network monitor with a
// permanently offline decision. Local Assistant must never open a network monitor or a
// connection, so reachability is never observed and offline mode is never negotiated.
// Removed: `import Network`, `NetworkMonitor` (NWPathMonitor + its dispatch queue),
// `HubApi.networkMonitor`, the `NetworkMonitor.shared.startMonitoring()` call in `init`,
// and `NetworkStateActor.update(path:)`.
extension HubApi {
    enum NetworkState {
        /// Always offline: every Hub fetch must be satisfied from local files or fail.
        static func shouldUseOfflineMode() -> Bool { true }
    }
}
