...
extension ModelUtilities {
    ...
    public static func loadTokenizer(
        ...
        // LocalAssistant-modification: removed the Hugging Face Hub download fallback.
        // The tokenizer ships inside the model folder, and the app must never request a
        // network connection, so a missing or unreadable tokenizer fails here instead of
        // being fetched from huggingface.co.
        throw WhisperError.tokenizerUnavailable(
            "No local tokenizer for \(tokenizerName). Reinstall the offline model package."
        )
    }
